...
```
# Basic vizualization
In this section, we will perfome sosme basic analyses of the dataset provided in Nitizan et al. using Seurat. We will visualize the cells in a reduced space using UMAP and visualize the expression of some genes. 
...
# optional geometry of the tissue (if you don;t have it you can assume your tissue is a rectangle )
datafile_geom   ='data/data_day2/bdtnp/geometry.txt'
# Optional : list of highly variable genes (to speed up computations)
...
* Nitizan et al. advised to use the **normalized gene expression matrix** (e.g. using the global scaling log-normalization method)
* The data need to be in the `Anndata` format. Novosparc has a function to import an expression matrix and to convert it in the `Anndata` format (alternativaly you can convert a pandas dataframe to an `Anndata`).
...
The function *create_target_space_from_image* creates a target space by using a user-generated image. The image has to be a black shape drawn onto a white background. You can do it in Fiji for instance.
```{python3}
